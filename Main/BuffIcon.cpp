...
{
	if (gProtect.m_MainInfo.CustomInterfaceType == 0 || gProtect.m_MainInfo.CustomInterfaceType == 2 || gProtect.m_MainInfo.CustomInterfaceType == 4 )
	{
...
void BuffIcon::sub_777EB0(float XIcon, float YIcon)
{
    ...
    float Height = (float)((28.0f / 100) * 50.5); 
    float Width = (float)((20.0f / 100) * 50.5); 
    float RenderW = (float)((256.0f / 100) * 50.5);
	...
	for( IndexTime = 0 ; IndexTime < MAX_EFFECT_LIST; ++IndexTime)
	{
	    ...
	    x = XIcon + v27 * (Width + 5.0) - 20;
		y = YIcon + v28 * (Height + 5.0) - 11;
		...
		RenderBitmap((BuffIndex - 1) / 80 + 31725, x, y, Width, Height,  v9, v10, v12, v11, 1, 1, 0.0); //okkk
