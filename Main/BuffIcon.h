...
public:
	...
	~BuffIcon();
	void Init();
	void LoadEng(RenderTooltipBuff * info);
	void LoadSpn(RenderTooltipBuff * info);
	void LoadPor(RenderTooltipBuff * info);

	static unsigned int __stdcall Timer(void *Args);
	static void __fastcall RenderIconBuff(struct_v15 * This, int a2);
	void sub_777EB0(float StartX, float StartY);
	...
	int LoadBuffCount;
	int UserBuffCount;
	bool CUSTOM_BUFFICON;
	HANDLE hThread;
	BUFF_USER UserBuff[MAX_EFFECT_LIST];
public:
	char RenderInfoBuff[MAX_BUFF_INDEX][256];
	std::map<int, RenderTooltipBuff> BuffEffectTRSData_ENG;
	std::map<int, RenderTooltipBuff> BuffEffectTRSData_SPN;
	std::map<int, RenderTooltipBuff> BuffEffectTRSData_POR;
}; extern BuffIcon gIconBuff;