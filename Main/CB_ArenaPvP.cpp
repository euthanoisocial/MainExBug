...
CBArenaPvP::CBArenaPvP()
{
	...
	for (int st = 0; st < 50; st++)
	{
		...
		GetPrivateProfileStringA("TextVN_ArenaPvP", GetFotText, "Null", TextVN[st], sizeof(TextVN[st]), "./Data/SPK/Message/Messenger.ini");
	}
}
...
void CBArenaPvP::DrawWindowNPC()
{
	...
	float StartX = (MAX_WIN_WIDTH / 2) - (CuaSoW / 2);
	float StartY = 100.0;
	gCentral.gDrawWindowCustom(StartX, StartY, CuaSoW, CuaSoH - 80, eWindowNPCArenaPvP, this->TextVN[0]);

	if (gCentral.gDrawButton(StartX + (CuaSoW / 2) - (115 / 2) + 100, StartY + (CuaSoH - 45) - 75, 75, 14, gConfig.TSPK_TEXT_ENGINE[15]) && (GetTickCount() - gInterface.Data[eWindowNPCArenaPvP].EventTick) > 300)
	{
		XULY_CGPACKET pMsg;
