...
void RenderWindowsOption(int a1)
{
	...
	*(signed int *)(This + 16) = 320 - (190 / 2); // Esto dice desde donde empieza las opcion
	...
	pDrawText(pTextThis(), x - 150, y + 10, gConfig.TSPK_TEXT_ENGINE[18], 250, 0, (LPINT)3, 0);
...
void RenderOptionText(int a1)
{
	...
	pDrawText(pTextThis(), x + 45, y + 100, gConfig.TSPK_TEXT_ENGINE[19], 0, 0, (LPINT)1, 0); //-- HP Monster
	//OPCIONES DERECHA
	pDrawText(pTextThis(), x + 158, y + 40, gConfig.TSPK_TEXT_ENGINE[20], 0, 0, (LPINT)1, 0); //-- HP Monster
	pDrawText(pTextThis(), x + 158, y + 60, gConfig.TSPK_TEXT_ENGINE[21], 0, 0, (LPINT)1, 0); //-- Time Bar
	pDrawText(pTextThis(), x + 158, y + 80, gConfig.TSPK_TEXT_ENGINE[22], 0, 0, (LPINT)1, 0); //-- Show MiniMap
	pDrawText(pTextThis(), x + 158, y + 100, gConfig.TSPK_TEXT_ENGINE[23], 0, 0, (LPINT)1, 0); //-- Show MiniMap
	//TAMAÑO DE FUENTE
	pDrawText(pTextThis(), x + 25, y + 285, gConfig.TSPK_TEXT_ENGINE[24], 0, 0, (LPINT)1, 0);
...
void RenderPane_Esc(DWORD id, float x, float y, float w, float h)
{
	...
	pSetBackgroundTextColor(pTextThis(), 0, 0, 0, 0);
	pSetTextColor(pTextThis(), 0xACu, 0xFFu, 0x38u, 0xFFu); 
	pDrawText(pTextThis(), x , y + 15, gConfig.TSPK_TEXT_ENGINE[25], 230, 0, (LPINT) 3, 0);
...
void Centralizado::gDrawWindowCustom(float StartX, float StartY, float CuaSoW, float CuaSoH, int eNumWindow, char* WindowName)
{
	...
	if (pCheckMouseOver(CloseX, CloseY, 36, 36) == 1)
	{
		...
		gInterface.DrawToolTip(gObjUser.m_CursorX + 20, gObjUser.m_CursorY, gConfig.TSPK_TEXT_ENGINE[26]);
	}
	gInterface.DrawFormat(eGold, StartX, StartY + 9, CuaSoW, 3, WindowName);
	pGLSwitch();
	EnableAlphaTest(0);
