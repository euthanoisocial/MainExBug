...
__declspec(naked) void SetItemEffect()
{
	...
	Item = gSmokeEffect.getSmokeTRSData(pItemType);
...
void ItemModelLoad() // OK
{
	...
	#if(HT_THUE_FLAG)
	//-------------------- custom flag
	for (std::map<int, MODEL_FLAG>::iterator it = gCustomModelFlag.mModelFlagData.begin(); it != gCustomModelFlag.mModelFlagData.end(); it++)
	{
		LoadItemModel(it->second.ModelIndex + ITEM_BASE_MODEL, "SPK\\Flag\\", it->second.ModelName, -1);
	}
	#endif
	...
	for(std::map<int,CUSTOM_ITEM_INFO>::iterator it = gCustomItem.m_CustomItemInfo.begin();it != gCustomItem.m_CustomItemInfo.end();it++)
	{
		LoadItemModel((it->second.ItemIndex+ITEM_BASE_MODEL),((it->second.ItemIndex>=GET_ITEM(7,0) && it->second.ItemIndex<GET_ITEM(12,0))?"Player\\":"Item\\"),it->second.ModelName,-1);
	}
	//-------------------- custom gloves
	...
	{
		//-------------------- custom Bow
		for(std::map<int,CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin();it != gCustomBow.m_CustomBowInfo.end();it++)
		{
			LoadItemModel((it->second.ItemIndex+ITEM_BASE_MODEL),"Item\\",it->second.ModelName,-1);
		}
	}
...
void ItemTextureLoad() // OK
{
	...
	for (std::map<int, MODEL_FLAG>::iterator it = gCustomModelFlag.mModelFlagData.begin(); it != gCustomModelFlag.mModelFlagData.end(); it++)
	{
		LoadItemTexture(it->second.ModelIndex + ITEM_BASE_MODEL, "SPK\\Flag\\", it->second.ModelName, -1);
	}
	#endif
	for(int n=0;n < MAX_CUSTOM_JEWEL;n++)
	{
	...
	for(std::map<int,CUSTOM_ITEM_INFO>::iterator it = gCustomItem.m_CustomItemInfo.begin();it != gCustomItem.m_CustomItemInfo.end();it++)
	{
		LoadItemTexture((it->second.ItemIndex+ITEM_BASE_MODEL),((it->second.ItemIndex>=GET_ITEM(7,0) && it->second.ItemIndex<GET_ITEM(12,0))?"Player\\":"Item\\"),it->second.ModelName,-1);
	}
	//-------------------- custom gloves
	if(gProtect.m_MainInfo.DisableCustomClaws == 0)
	{
