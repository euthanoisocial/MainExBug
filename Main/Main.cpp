...
Naked(Credit)
{
	gInterface.DrawFormat(eWhite, 35, 463, 150, 1,	"Cập nhật cuối: 22:00:00 PM (25/06/2024)");
	//debug 
	//char IsValue[255];
	...
	//gInterface.DrawFormat(eWhite, 35, 463, 150, 1, IsValue);
	//-----------
	//tue add to test
	if (gProtect.m_MainInfo.CustomNewsEnable == 1)
	{
...
void RunMyFuncTion()
{
	...
	//tue tạm thời comment dòng này
	//SetOp((LPVOID)0x004D7D13, (LPVOID)Credit, ASM::JMP);	//Tạo dòng CopyRight
	//tue add to test
	//SetOp((LPVOID)0x004D7D13, (LPVOID)Notices2, ASM::JMP);
	///////////////////////////////////////////////////////////////////////////////////////////
...
{	
	...
	if(gProtect.ReadMainFile("SPK/Loader.spk") == 0)
	//if(gProtect.ReadMainFile("WZ/Loader.dll") == 0)
	{
		MessageBoxW(NULL, L"Khong tin thay SPK/Loader.spk , contact to administrator for help!\nZalo: 0775 838 858\nError Code: 0x0000000A", L"SuperKhung", MB_OK);
		ExitProcess(0);
	}

	if(gProtect.ReadTooltipTRSData( "Data\\Local\\TooltipTRSData.bmd" ) == 0)
	{
		MessageBoxW(NULL, L"Library was not found, contact to administrator for help!\nZalo: 0775 838 858\nError Code: 0x0000000B", L"SuperKhung", MB_OK);ExitProcess(0);
	}

	if(gProtect.ReadMakeViewTRSData("Data\\Local\\MakeWorldTRSData.bmd") == 0)
	{
		MessageBoxW(NULL, L"Library was not found, contact to administrator for help!\nZalo: 0775 838 858\nError Code: 0x0000000C", L"SuperKhung", MB_OK);ExitProcess(0);
	}

	if(gProtect.ReadPetEffectBMD("Data\\Local\\EffectTRSData.bmd") == 0)
	{
		MessageBoxW(NULL, L"Library was not found, contact to administrator for help!\nZalo: 0775 838 858\nError Code: 0x0000000D", L"SuperKhung", MB_OK);ExitProcess(0);
	}
	#if(ANTI_CRACK_MAIN == 1)
	MainProtection();
	#endif
	...
	SetCompleteHook(0xE9, 0x004D1CF0, &DumpExceptionFilter); //-- tue add file dump
	//===================================================================================================
	...
	//add Notice Select, Tue
	gCustomNoticeInfo.Load(gProtect.m_MainInfo.CustomNoticeInfo);
	//--------------------------
	//add Slide, Tue
	gSlide.Load();
	//--------------------------
	//add -> Sky Box & Day Night, Tue
	gSkyBox.Load();
	//--------------------------
	...
	//-----------
	//InitNoticesTue();
	if (gProtect.m_MainInfo.EnableNoticeSystem == 1)
	{
		InitNoticesTue();
		//InitNotices();
		//InitNotices();
	...
	{
		gEmojis.AddTextures(); //Emojis Sistem	
	}
	//tue add
	giPetManager::InitDarkSpirit();
}
void SPK_HideMain(HINSTANCE hModule)
{
...
//tue add fixxx to file dump error //---------------
