...
// Khởi tạo ButterFly và ButterMuun

BYTE ButterFly[6480] = { 0 };
BYTE ButterMuun[648 * MAX_BUTTERFLES] = { 0 };
#if(SLOTPET2)
...
// Khai báo biến CRITICAL_SECTION toàn cục
CRITICAL_SECTION logfile_cs;

// Hàm khởi tạo CRITICAL_SECTION (gọi ở đâu đó trước khi sử dụng)
void InitLogfileCriticalSection() {
	InitializeCriticalSection(&logfile_cs);
}

// Hàm hủy CRITICAL_SECTION (gọi ở đâu đó sau khi sử dụng xong)
void DeleteLogfileCriticalSection() {
...
void MoveMuunBugs()
{
	// Kiểm tra ButterFly và ButterMuun có hợp lệ không
	if (ButterFly == nullptr || ButterMuun == nullptr) {
		// Ghi log lỗi
		EnterCriticalSection(&logfile_cs);
		std::ofstream logfile("muun_debug.log", std::ios::app);
		if (logfile.is_open()) {
			logfile << "ButterFly or ButterMuun is null!" << std::endl;
			logfile.close();
		}
	...
	std::ofstream logfile("muun_debug.log", std::ios::app);
	if (logfile.is_open()) {
		logfile << "MoveMuunBugs called" << std::endl;
		logfile << "ButterFly address: " << ButterFly << std::endl;
		logfile << "ButterMuun address: " << ButterMuun << std::endl;
		logfile << "MAX_BUTTERFLES: " << MAX_BUTTERFLES << std::endl;
		logfile.close();
	}
	//LeaveCriticalSection(&logfile_cs);
...
void RenderMuunBugs()
{
	...
	std::ofstream logfile("muun_debug.log", std::ios::app);
	if (logfile.is_open()) {
		logfile << "RenderMuunBugs called" << std::endl;
		logfile << "ButterFly address: " << ButterFly << std::endl;
		logfile << "ButterMuun address: " << ButterMuun << std::endl;
		logfile << "MAX_BUTTERFLES: " << MAX_BUTTERFLES << std::endl;
		logfile.close();
	}
	// Kiểm tra MAX_BUTTERFLES có hợp lệ không
	if (MAX_BUTTERFLES <= 0) {
		return;
	}
	...
	// Lặp qua ButterFly
	for (int i = 0; i < MAX_BUTTERFLES; i++) {
		// Kiểm tra phạm vi của mảng
		if (i >= MAX_BUTTERFLES) {
			break;
		}

		// Gọi RenderPetScale với ButterFly
		if (RenderPetScale((int)&ButterFly[648 * i], false) == FALSE) {
			break;
		}
	}
	...
	for (int i = 0; i < MAX_BUTTERFLES; i++) {
		...
		// Gọi RenderPetScale với ButterMuun
		if (RenderPetScale((int)&ButterMuun[648 * i], false) == FALSE) {
			break;
		}
	}
...
void SetCharacterEffectEquip(int c)
{
	...
	if (logfile.is_open()) {
		...
		logfile << "v6 address: " << v6 << std::endl;
		logfile.close();
	}
	...
	if (*(DWORD*)(c + 824) == 1163)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (logfile.is_open()) {
				logfile << "Character_Machin: " << Character_Machin << std::endl;
				logfile.close();
			}
			if (*(WORD*)v6 == 0xFFFF)
			{
...
void SetCharacterEffectEquip(int c)
{
	...
	if (logfile.is_open())
	{
		time_t now = time(0);
		char* dt = ctime(&now);
		logfile << "SetCharacterEffectEquip called at: " << dt;
	...
	if (*(DWORD*)(c + 824) == 1163)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (*(WORD*)v6 == 0xFFFF)
			{
				...
				if (logfile.is_open())
				{
					logfile << "v6 after CharacterMachine_Equipment: " << v6 << std::endl;
				}
			}
		...
		if (logfile.is_open())
		{
			logfile << "EQUIPMENT_HELPER v6: " << v6 << std::endl;
		}
		if (*(WORD*)v6 == 0xFFFF)
		{
			*(WORD*)(c + 556) = 0xFFFF;
		...
		if (logfile.is_open())
		{
			logfile << "Equipment[12] v6: " << v6 << std::endl;
		}
		if (*(WORD*)v6 == 0xFFFF)
		{
			*(WORD*)(c + 592) = 0xFFFF;
		...
		for (int i = EQUIPMENT_HELM; i <= EQUIPMENT_BOOTS; i++)
		{
			...
			if (logfile.is_open())
			{
				logfile << "EQUIPMENT loop i: " << i << ", v3 before check: " << v3 << std::endl;
			}
			...
			if (*(WORD*)v3 == 0xFFFF)
			{
				...
				if (logfile.is_open())
				{
					logfile << "v3 after CharacterMachine_Equipment: " << v3 << std::endl;
				}
			}

			if (logfile.is_open())
			{
				logfile << "EQUIPMENT loop i: " << i << ", v3 after all checks: " << v3 << std::endl;
			}
...
void CNewUIMyInventory::SetEquipmentSlotInfo(UIMyInventory* This)
{
	...
	m_Slots[EQUIPMENT_PENTAGRAM2 - 12].width = 18;
	m_Slots[EQUIPMENT_PENTAGRAM2 - 12].height = 18;
	...
	m_Slots[EQUIPMENT_ERRING_RIGHT - 12].width = 18;
	m_Slots[EQUIPMENT_ERRING_RIGHT - 12].height = 18;
	...
	m_Slots[EQUIPMENT_ERRING_LEFT - 12].width = 18;
	m_Slots[EQUIPMENT_ERRING_LEFT - 12].height = 18;
	m_Slots[EQUIPMENT_ERRING_LEFT - 12].dwBgImage = 65537;
#endif
	//--
#if (MAX_SLOT_MACHINE > EQUIPMENT_STONE_MAGIC)
	m_Slots[EQUIPMENT_STONE_MAGIC - 12].x = m_Pos.x + 118;
	m_Slots[EQUIPMENT_STONE_MAGIC - 12].y = m_Pos.y + 87;
...
BYTE PetEquipedHPBar(int iX, int iY, int mode)
{
	...
	else
	{
		...
		CreateBarHP(StartPosX, iY + 554, PetName, PetHP, 255, 0);
		result = 1;
	}
...
BYTE RenderEquipPetLife(int sx, int sy)
{
	...
	if (Type != 0xFFFF)
	{
		...
		CreateBarHP(StartPosX, sy + 554, Attribute->Name, Dur, 255, 0);
		t = 1;
	}
	else
	{
...
{
	if (InChaosCastle(World) != true && (*(DWORD*)(Owner + 48) == MODEL_PLAYER || Type == MODEL_HELPER))
	{
		for (int i = 0; i < MAX_BUTTERFLES; i++)
		{
