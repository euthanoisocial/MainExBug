...
void cOffTrade::DrawOffTradeWindowEx700(float x, float y)
{
	...
	if(gOffTrade.ShopActive != 0)
	{
		...
		if( gProtect.m_MainInfo.CustomOffStoreEnable )
		{
			...
			//--
			gInterface.DrawFormat(eYellow, Return2X(x + TransForX(40)), y + 320, Return2X(TransForX(108)), 3, gConfig.TSPK_TEXT_ENGINE[111]);
			gInterface.DrawFormat(eYellow, Return2X(x + TransForX(40)), y + 360, Return2X(TransForX(108)), 3, gConfig.TSPK_TEXT_ENGINE[112]);
		}
		else
		{
			gInterface.Data[eSTORE_CLOSE].OnShow = true;
	...
	if(gOffTrade.ShopActive != 0)
	{
		if(gProtect.m_MainInfo.CustomOffStoreEnable)
		{
			if (gInterface.IsWorkZone(eSTORE_CLOSE))
			{
				...
				gInterface.DrawToolTip(v6 + 60, 300, gConfig.TSPK_TEXT_ENGINE[111]);
			}
			else if (gInterface.IsWorkZone(eOFFTRADE_OPEN))
			{
				...
				gInterface.DrawToolTip(v6 + 60, 340, gConfig.TSPK_TEXT_ENGINE[112]);
			}
			else
			{
				gInterface.DrawButtonRender(eSTORE_CLOSE, v6 + 40,310, 0, 0);
			...
			gInterface.DrawFormat(eYellow, v6 + 40, 320, 108, 3, gConfig.TSPK_TEXT_ENGINE[111]);

			gInterface.DrawFormat(eYellow, v6 + 40, 360, 108, 3, gConfig.TSPK_TEXT_ENGINE[112]);
		...
		else
		{
			if (gInterface.IsWorkZone(eSTORE_CLOSE))
			{
				...
				gInterface.DrawToolTip(v6 + 60, 320, gConfig.TSPK_TEXT_ENGINE[111]);
			}
			else
			{
				gInterface.DrawButtonRender(eSTORE_CLOSE, v6 + 40,330, 0, 0);
			}
			gInterface.DrawFormat(eYellow, v6 + 40, 340, 108, 3, gConfig.TSPK_TEXT_ENGINE[111]);
...
void cOffTrade::ShowTwoString(DWORD a1, const char *a2)
{
	...
	{
		pSetItemTextLine(a1, gConfig.TSPK_TEXT_ENGINE[113]);
	}
}
