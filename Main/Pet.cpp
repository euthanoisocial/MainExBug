...
char CustomPet::PetHPBar(int a1, int a2)
{
	...
	else
	{
		...
		if( gProtect.m_MainInfo.CustomInterfaceType == 3 )
		{
			...
			#else
				sub_7DFA60(3, gInterface.PetY + 5, (int)&PetName, PetHP, 255, 0);
			#endif
		}
		else if( gProtect.m_MainInfo.CustomInterfaceType != 1 && gProtect.m_MainInfo.CustomInterfaceType != 2 )
		{
				...
				sub_7DFA60(3, gInterface.PetY + 80, (int)&PetName, PetHP, 255, 0);
			#else
				sub_7DFA60(3, gInterface.PetY - 5, (int)&PetName, PetHP, 255, 0); // <<<< a nham =))
			#endif
		}
		result = 1;
	}
...
#endif
{
	// Kiểm tra a1 có hợp lệ không
	if (a1 == 0) {
		// Ghi log lỗi hoặc xử lý phù hợp
		return false;
	}

	// Kiểm tra các offset quan trọng
	if (*(BYTE*)(a1 + 4) == 0) {
		// Xử lý phù hợp
		return false;
	}
	...
	// Kiểm tra con trỏ (a1 + 472)
	DWORD* ptr = (DWORD*)(a1 + 472);
