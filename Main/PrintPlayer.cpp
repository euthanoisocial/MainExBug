...
void DrawHp(float a1, float a2, int Value, float a4)
{
	...
	{
		pDrawBar(a1 - 15, 445.0f, ViewCurHP, HPSize);
	}
	else
	{
		pDrawBar(a1, 445.0f, ViewCurHP, HPSize);
	}
}

void DrawMp(float a1, float a2, int Value, float a4)
{
	pDrawBar(a1, 445.0f, ViewCurMP, HPSize);
}
#endif

#if(DAMAGE_RENDER_ENABLE)

void PrintDamageOnScreenHP(vec3_t Position, DWORD Value, vec3_t Color, float scale, bool bMove) // OK
{
	if (((int)Value) > 0)
	{
		Value = ViewDamageHP;
	}
	g_DamageRender.SetDmgTarget(Position, Color, (int)(ViewDamageTYPE), scale, ViewDamageHP);
	ViewDamageTYPE = 0;
}

void PrintDamageOnScreenSD(vec3_t Position, DWORD Value, vec3_t Color, float scale, bool bMove) // OK
{
	if (((int)Value) > 0)
	{
		Value = ViewDamageHP;
...
void PrintDamageOnScreenRF(vec3_t Position, DWORD Value, vec3_t Color, float scale, bool bMove) // OK
{
...
void PrintTextReplace(DWORD* value, DWORD type) // OK
{
	switch (value[0])
	{
		...
		switch (type)
		{
			case 1:			value[1] = ViewDarkSpiritAttackSpeed;			break;
			case 2:			value[1] = ViewPhysiSpeed;						break;
			case 3:			value[1] = ViewDarkSpiritAttackSpeed;			break;
			case 4:			value[1] = ViewDarkSpiritAttackSpeed;			break;
		}
		break;
		case 170: 		GetViewMagicDamage(&value[1], &value[2]);			break;
	case 203: 
		switch (type)
		{
			case 1:
