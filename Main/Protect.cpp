...
bool CProtect::ReadMainFile(char* name) // OK
{
	...
	{
		//CloseHandle(file);
		MessageBoxW(NULL, L"GetFileSize(file,0) != sizeof(MAIN_FILE_INFO)", L"SuperKhung", MB_OK);
		//return 0;
	}
	...
	{
		MessageBoxW(NULL, L"ReadFile(file,&this->m_MainInfo,sizeof(MAIN_FILE_INFO),&OutSize,0)", L"SuperKhung", MB_OK);
		//CloseHandle(file);
		//return 0;
	}
