...
void CRegisterForm::RequestRegister(LPVOID pClass)
{
	...
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_USERID;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[175]);
	}
    ...
    {
        szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
        PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[176]);
    }
	else if (strlen(szCaptcha) <= 0)
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[177]);
	}
	else if (strcmp(szth->reCaptcha, szCaptcha) != NULL)
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[178]);
	}
	else
	{
...
void CRegisterForm::RequestAccount(BYTE code)
{
	...
	switch (code)
	{
		case CREATE_ACCOUNT_FAIL_ID:			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[179]);			break;
		case CREATE_ACCOUNT_SUCCESS:			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[180]);			break;
		case CREATE_ACCOUNT_FAIL_RESIDENT:		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[181]);			break;
		default:								PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, gConfig.TSPK_TEXT_ENGINE[182]);			break;
	}
}
#endif