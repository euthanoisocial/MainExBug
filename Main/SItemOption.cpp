...
void CSItemOption::ItemTooltipS15(float PosX, float PosY, float Width, float Height, float Arg5, int Arg6)
{
	...
	if(gProtect.m_MakeViewTRSData.TooltipS15 == 0)
	{
		glEnable(0xDE1u);
...
void CSItemOption::RenderColorItem( int ColorTexto )
{
...
#if LUC_CHIEN
#define	CSItemOptionRenderDefaultOptionText  ((int(__thiscall*)(int This,ObjectItem *a2, int a3)) 0x004EADD0)
int __thiscall CSItemOption::RenderDefaultOptionText(int This, ObjectItem* a2, int a3)
{
	if (a2->ItemID != -1)
	{
		a3 = GInfo.nInformationOP(a3, a2);
	}

	return CSItemOptionRenderDefaultOptionText(This, a2, a3);
}
#endif
void ItemTooltipRender(int x, int y, int nCount, int witd, int height,int type, int Align)
{
	...
	cRenderH = ( (gPostInterface.item_post_ && pCursorX < 180) ? ( SellItem + ( cRenderH / ( pWinHeightReal / v71 ))) : ( cRenderH / ( pWinHeightReal / v71 )));
	...
	if ( Align == 1 && nCount > 0 )
	{
		...
		if ( gPostInterface.item_post_ && pCursorX < 180 )
		{
			gPostInterface.DrawItemIMG(cRenderX + ( cRenderW / 2 ) - ( (SellItem - 20) / 2 ) , cRenderY, SellItem - 20, SellItem - 20 , gObjUser.PostItemID,gObjUser.PostItemLV, gObjUser.PostItemExc, gObjUser.PostItemAcc, 0);
		}
	}
	//-- Escritura
	for( int k = 0; k < nCount; ++k )
	{
		...
		if( ((DWORD) TooltipLine[k] != 10 && ( DWORD) TooltipLine[k] != 32 || (DWORD) TooltipLine[k] ) && lstrlenA(TooltipLine[k]) > 2 )
		{
			...
			if( ( k > 0 ) && ( gPostInterface.item_post_ && pCursorX < 180 ) )
			{
...
{
	if (itemindex < 0 || itemindex >= MAX_ITEM2)
	{
		return 0;
	}
...
{
	#if LUC_CHIEN
	SetCompleteHook(0xE8, 0x007E56DB, &CSItemOption::RenderDefaultOptionText); //ok
	#endif
