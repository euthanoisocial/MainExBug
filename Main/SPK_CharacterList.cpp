...
SPKCharacterList gCharList;

void SPKCharacterList::ImgLoad()
{
	pLoadImage("Custom\\SuperKhung\\Other\\CharSelectBg.tga"			, 0x18909, GL_LINEAR, GL_REPEAT, 1, 0);
}

void SPKCharacterList::StatusInfo(float X,float Y,int a1, float a2)
{
	char* Name;
	char* Status;
	int Class;
...
void SPKCharacterList::GetPosForm()
{
...
void SPKCharacterList::SPK_CharInfoLoad(int X, int Y , int mHeroID )
{
...
void SPKCharacterList::SelectChar()
{
	float XBPCoord = TransFor2X(320) - TransForX(320);	
	if ( !*(BYTE *)((int)sub_482B70() + 33948) )
	{
		SPK_CharInfoLoad(Return2X(XBPCoord + TransForX(90)),  160,0); //-- Char 1/5
		SPK_CharInfoLoad(Return2X(XBPCoord + TransForX(190)), 160,1); //-- Char 2/5		
		SPK_CharInfoLoad(Return2X(XBPCoord + TransForX(290)), 160,2); //-- Char 3/5	
		SPK_CharInfoLoad(Return2X(XBPCoord + TransForX(385)), 160,3); //-- Char 4/5	
		SPK_CharInfoLoad(Return2X(XBPCoord + TransForX(480)), 160,4); //-- Char 4/5				
	}
}