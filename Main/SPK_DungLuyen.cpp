...
void MixItemDungLuyen::DrawWindows()
{
	...
	float StartX = 10;
	float StartY = 50; // 0911105253
	float WindowW = 280;
	float WindowH = 400;

	RenderBitmap(0x1791E, StartX, StartY + 9, WindowW - 40, WindowH, 0, 0, 1, 1, 1, 1, 0.0);
	RenderBitmap(0x17923, StartX, StartY + 10, WindowW - 40, WindowH - 330, 0, 0, 1, 1, 1, 1, 0.0);
	if (pCheckMouseOver(StartX + 203, StartY + 18, 12, 12) == 1)
	{
	...
	CustomFont.Draw((HFONT)pFontBold, PosXInvetoryImg + 5,  PosYInvetoryImg - 10, 0xEBA000FF, 0x0, 52, 0, 3, "Item Chính");
	CustomFont.Draw((HFONT)pFontBold, PosXInvetoryImg + 5,  PosYInvetoryImg + 90, 0xEBA000FF, 0x0, 52, 0, 3, "Kết Quả");
	CustomFont.Draw((HFONT)pFontBold, PosXInvetoryImg + 70, PosYInvetoryImg - 10, 0x00FBFF69, 0x0, 121, 0, 3,"Item Phụ");
	...
	for (int n = (VipCharListPage * DungLuyenListMaxPerPage); n < DataListItem; n++)
	{
		...
		if (gElemental.gDrawButton(PosXInvetoryImg + 120, PosYInvetoryImg + 165, 80, 11, "Dung Luyện", 70))
		{
			if (gMixDL.DL_ActiveMix)
			{
