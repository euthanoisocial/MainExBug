...
void SPK_NPC_TRADER::SendToProtocol6A(BYTE* lpMsg)
{
...
void SPK_NPC_TRADER::SendToProtocol6B(BYTE* lpMsg)
{
...
void SPK_NPC_TRADER::DrawInfoItemSet()
{
...
void SPK_NPC_TRADER::BackDoiItem(int Type)
{
...
void SPK_NPC_TRADER::DrawItemSlotSelect(float X, float Y, float W, float H, BYTE ItemSlot)
{
	if (DoiItem == ItemSlot)
	{
...
void SPK_NPC_TRADER::DrawItemSlotToolTip(float X, float Y, float W, float H, BYTE ItemSlot)
{
	ObjectItem* SecondItemSlot = &*(ObjectItem*)(BPConvertByteItem(*(DWORD*)(GetInstance() + 36), this->ItemKetQua[ItemSlot - 1]));
	if (SecondItemSlot->ItemID > 0)
	{
...
#include "OffTrade.h"
void SPK_NPC_TRADER::DrawDoiItem()
{
	...
	gCentral.gDrawWindowCustom(StartX, StartY, WindowW, WindowH - 137, eWindowDoiItem, gConfig.TSPK_TEXT_ENGINE[314]);
	{
		float BoxItemWH = 35;
		//===Item Chinh
		...
		CustomFont.Draw((HFONT)pFontBold, PosXGroup1 - 8, PosYGroup1 - 5, 0xEBA000FF, 0x00FBFF69, 124, 0, 3, gConfig.TSPK_TEXT_ENGINE[315]);//
		...
		CustomFont.Draw((HFONT)pFontBold, PosXGroup1 - 8, PosYTextInfo - 53, 0xFFFFFFFF, 0x00FBFF69, 124, 0, 3, gConfig.TSPK_TEXT_ENGINE[316]);//

		CheckCoinItem(this->WCoinC,		PosXGroup1, PosYTextInfo - 35, gConfig.TSPK_TEXT_ENGINE[10]);
		CheckCoinItem(this->WCoinP,		PosXGroup1, PosYTextInfo - 25, gConfig.TSPK_TEXT_ENGINE[11]);
		CheckCoinItem(this->WCoinG,		PosXGroup1, PosYTextInfo - 15, gConfig.TSPK_TEXT_ENGINE[9]);
		CheckCoinItem(this->CoinAtm,	PosXGroup1, PosYTextInfo - 5, gConfig.TSPK_TEXT_ENGINE[12]);

		if (gCentral.gDrawButton(PosXGroup1 - 4, PosYGroup1 + 180, 115, Main_Font_Height, gConfig.TSPK_TEXT_ENGINE[317]))
		{
			PlayBuffer(25, 0, 0);
			BackDoiItem(DoiItem);
			DoiItem = -1;
