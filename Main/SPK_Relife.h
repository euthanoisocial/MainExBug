...
class SpkRelife
{
	...
	void SendMenuButton();
	void OpenWindowsRelife() { gInterface.Data[SpkRelife_MainBox].OnShow = true; pSetCursorFocus = true; };
	void CloseWindowsRelife() { gInterface.Data[SpkRelife_MainBox].OnShow = false; pSetCursorFocus = false; };
	bool CheckWindowsRelife() { return gInterface.Data[SpkRelife_MainBox].OnShow; };
	void CallWindowsRelife();
	bool MainWindowsRelife(DWORD Event);
	bool RejectWindowsRelife(DWORD Event);
	void TaiSinhCheckRecv(RelifeCheckUpdate* SPK);
	void TaiSinhDiemRecv(RelifePointUpdate* SPK);
	void SaveFormGs(RelifeCheckReqInfoPart1* SPK);
