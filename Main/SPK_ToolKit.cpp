...
void SPK_ToolKitMain::CreateFontNew(int X, int Y, int W, int Pos, int Size, int Bold, LPCSTR String, COLORREF TextColor, HFONT CrateSub)
{
...
void SPK_ToolKitMain::MakeShadowText(int X, int Y, int W, int Pos, LPCSTR String,...)
{
	pSetBackgroundTextColor(pTextThis(), 0, 0, 0, 0);
	SetTextColorByHDC(pTextThis(), pMakeColor(0xFFu, 0xFFu, 0xFFu, 0xFFu));
	g_pRenderText.CRenderText(X, Y, String, W, 0, Pos, 0);
}
char* SPK_ToolKitMain::BGetItemName(int ItemType, int Level)
{
...
void SPK_ToolKitMain::RenderMixEffect(int type, int x, int y, int w, int h)
{
	int result; // eax
...
char* SPK_ToolKitMain::QNumberFormat(QWORD Number)
{
...
char* SPK_ToolKitMain::NumberFormat(DWORD Number)
{
	if (Number < 0) { return "0"; }
...
float SPK_ToolKitMain::RenderExNumberLevel2(float x, float y, int iNum, int iNum1, float fScale)
{
	EnableAlphaTest(1);
	if (fScale < 0.3f)
	{
...
void SPK_ToolKitMain::DrawButtonIcon( int Enum, int ImgID,float X, float Y, LPCSTR Text)
{
	...
	if (gInterface.IsWorkZone(Enum))
	{
		...
		if (gInterface.Data[Enum].OnClick) {
			RenderBitmap(ImgID, X, Y, 15, 18, 0.0, 0.451f, 0.878, 0.225, 1, 1, 0.0);}else{
			RenderBitmap(ImgID, X, Y, 15, 18, 0.0, 0.2255f, 0.878, 0.225, 1, 1, 0.0);}}else{
			RenderBitmap(ImgID, X, Y, 15, 18, 0.0, 0.0, 0.878, 0.225, 1, 1, 0.0);
	}
}
void SPK_ToolKitMain::DrawButtonIconRank(int Enum, int ImgID, float X, float Y, LPCSTR Text)
{
	gInterface.DrawButtonRender(Enum, X, Y, 0, 0.0f);
	if (gInterface.IsWorkZone(Enum))
	{
		...
		if (gInterface.Data[Enum].OnClick) {
			RenderBitmap(ImgID, X, Y, 16, 19, 0.0, 0.451f, 0.878, 0.225, 1, 1, 0.0);}else {
			RenderBitmap(ImgID, X, Y, 16, 19, 0.0, 0.2255f, 0.878, 0.225, 1, 1, 0.0);}}else {
			RenderBitmap(ImgID, X, Y, 16, 19, 0.0, 0.0, 0.878, 0.225, 1, 1, 0.0);
	}
}
float SPK_ToolKitMain::CRenderNumberClassic(BYTE Color, float x, float y, int iNum, float fScale)
{
	EnableAlphaTest(1);
	if (fScale < 0.3f)
	{
...
double SPK_ToolKitMain::CRenderNumberExBank(float a1, float a2, int a3, float a4, DWORD ModelID)
{
	double result;
	float v5;
	float v6; 
...
double SPK_ToolKitMain::CRenderNumberEx(float a1, float a2, double a3, float a4, DWORD ModelID)
{
	double result;
	float v5;
	float v6;
...
double SPK_ToolKitMain::CRenderNumberExNew(float a1, float a2, double a3, int a5, float Scale)
{
	double result;
	float v5;
	float v6;
	...
	if (Scale >= 0.300000011920929)
	{
		...
		{
			v6 = (double)(v10[i] - 48) * 12.0 / 128.0;
			v5 = (Scale - 0.400000011920929) * 16.0;
...
void SPK_ToolKitMain::RunAntiHack()
{
	if (SKILL_001)
	{
		MessageBoxW(NULL, L"Phát hiện tốc độ bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_002)
	{
		MessageBoxW(NULL, L"Phát hiện skill hồi phục bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_003)

	{
		MessageBoxW(NULL, L"Phát hiện sát thương không ổn định. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_004)
	{
		MessageBoxW(NULL, L"Phát hiện tốc độ di chuyển không đúng. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_005)
	{
		MessageBoxW(NULL, L"Không nhận diện được bị choáng, giật lùi của hiệu ứng. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_006)
	{
		MessageBoxW(NULL, L"Không nhận diện được hiệu ứng. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_007)
	{
		MessageBoxW(NULL, L"Phát hiện Hit bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_008)
	{
		MessageBoxW(NULL, L"Phát hiện đanh liền skill bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_009)
	{
		MessageBoxW(NULL, L"Phát hiện ra đòn không đúng vị trí. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_010)
	{
		MessageBoxW(NULL, L"Phát hiện Hit bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	...
	if (SKILL_011)
	{
		MessageBoxW(NULL, L"Phát hiện cỡi sói bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}

	if (SKILL_012)
	{
		MessageBoxW(NULL, L"Phát hiện tốc đánh bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	...
	if (SKILL_014)
	{
		MessageBoxW(NULL, L"Phát hiện tốc độ tấn công không đúng. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_015)
	{
		MessageBoxW(NULL, L"Phát hiện sát thương bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
	if (SKILL_016)
	{
		MessageBoxW(NULL, L"Phát hiện tốc độ bất thường. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	...
	if (SKILL_017)
	{
		MessageBoxW(NULL, L"Phát hiện đánh quá xa giới hạn. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}

	if (SKILL_018)
	{
		MessageBoxW(NULL, L"Phát hiện hành động bị loại bỏ. Vui lòng khởi động lại game!", L"SuperKhung", MB_OK | MB_ICONERROR); ExitProcess(0);
	}
}
