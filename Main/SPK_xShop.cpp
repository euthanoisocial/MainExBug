...
void CNewCashShop::DrawShop()
{
	...
	gCentral.gDrawWindowCustom(StartX, StartY, WindowW - 20, WindowH, eCustomShop, gConfig.TSPK_TEXT_ENGINE[318]);
	{
		pSetBlend(true);
		...
		CustomFont.Draw(CustomFont.FontBold, InfoMocNapX, StartY + 285, 0xEBA000FF, 0, WindowW - 20, 0, 3, gConfig.TSPK_TEXT_ENGINE[319]);
		...
		CustomFont.Draw(CustomFont.FontBold, InfoMocNapX + 1, InfoMocNapY - 2, 0xFFFFFFFF, 0x00FBFF69, 90, 0, 3, gConfig.TSPK_TEXT_ENGINE[320]);
		CustomFont.Draw(CustomFont.FontBold, InfoMocNapX + 96, InfoMocNapY - 2, 0xFFFFFFFF, 0x00FBFF69, WindowW - 145, 0, 3, gConfig.TSPK_TEXT_ENGINE[321]);
		...
		for (int X = 0; X < 5; X++)
		{
			...
			gInterface.SPKText(eWhite, InfoMocNapX - 69.02 + (80 * (X + 1)), InfoMocNapY - 24, 90, 3, this->MemCoin[X]);
		}
