...
void cThueFlag::Draw()
{
	...
	gCentral.gDrawWindowCustom(this->x, this->y, CuaSoW, CuaSoH, eMAIN_ThueFlag, gConfig.TSPK_TEXT_ENGINE[286]);

	if (pIsKeyRelease(VK_LBUTTON) && pCheckMouseOver(x + w - 25, y + 5, 20, 18))
	{
	...
	this->DrawButton(32331, x + 14 + 75 * 4, y + h - 100, 73, 27, 128.0, 128.0, ThueFlag.DisableAction);
	CustomFont.Draw(CustomFont.FontNormal, x + 14 + 75 * 4, y + h - 92.5, 0xffffffff, 0x0, 73, 0, 3, (DisableFlag == 1) ? gConfig.TSPK_TEXT_ENGINE[287] :gConfig.TSPK_TEXT_ENGINE[288]);
	...
	CustomFont.Draw(CustomFont.FontNormal, x + 14 + 75 * 4, y + h - 32.5, 0xffffffff, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[289]);

	if (this->Selected > 0)
	{
		this->DrawButton(32331, x + 14 + 75 * 3, y + h - 40, 73, 27, 128.0, 128.0, ThueFlag.PrevAction);
		CustomFont.Draw(CustomFont.FontNormal, x + 14 + 75 * 3, y + h - 32.5, 0xffffffff, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[290]);
	}

	RenderBitmap(31749, x + 20, y + 50, (CuaSoW / 4) + 10, (CuaSoH / 2) - 10, 0, 0, 173.0 / 256.f, 87.0 / 128.f, 1, 1, 0.0);
	RenderBitmap(31749, x + 20, y + CuaSoH - 85, (CuaSoW / 4) + 10, (CuaSoH / 4) - 5, 0, 0, 173.0 / 256.f, 87.0 / 128.f, 1, 1, 0.0);


	if (skin != 0)
	{
		//chi so
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 65, 0x66FF66FF, 0x0, 150, 0, 1,  gConfig.TSPK_TEXT_ENGINE[291], skin->Value1);
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 85, 0x66FF66FF, 0x0, 150, 0, 1,  gConfig.TSPK_TEXT_ENGINE[292], skin->Value2);
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 105, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[293], skin->Value3);
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 125, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[294], skin->Value4);
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 145, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[295], (skin->Time) / 60);

		if (gCustomModelFlag.CheckFlagModelIsLoad(gProtect.m_MainInfo.m_mModelFlagData[skin->Id].ModelIndex))
		{
			BBDrawItem3D(x + 10 + 25 * 4 + 100, y + 105, 50.f, 60.f,
				gProtect.m_MainInfo.m_mModelFlagData[skin->Id].ModelIndex, 0, 0, 0, 0, 0.8f, true);
		}
		//yeu cau
		if (skin->Time < 1)
		{
			CustomFont.Draw(CustomFont.FontNormal, x, y + 205, 0xFF3300FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[296], gToolKit.NumberFormat(skin->Price));
		}
		else
		{

			CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 205, 0xFF3300FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[296], gToolKit.NumberFormat(skin->Price));
		}
		CustomFont.Draw(CustomFont.FontNormal, x + 30, y + 225, 0xFF3300FF, 0x0, 100, 15, 1, gConfig.TSPK_TEXT_ENGINE[297], skin->Value7);

		gInterface.DrawItem2(x + 80, y + 240, 15, 15, ITEM(skin->Value8, skin->Value9), 11, 0, 0, 0); 


		if (skin->Value10 != -1)
		{
			CustomFont.Draw(CustomFont.FontNormal, x + 80 * 2, y + 35, 0x0, 0x00F5FF, 150, 10, 0, " ");
			if (skin->Value10 > 0)
			{
				time_t timestamp = skin->Value10;
				std::string dateTimeString = convertUnixTimestampToString(timestamp);
				CustomFont.Draw(CustomFont.FontNormal, x + 80 * 2, y + 35, 0xffffffff, 0x0, 150, 0, 3, gConfig.TSPK_TEXT_ENGINE[298], strdup(dateTimeString.c_str()));
			}
			else
			{
				CustomFont.Draw(CustomFont.FontNormal, x + 4 + 75 * 2, y + 35, 0xffffffff, 0x0, 150, 0, 3, gConfig.TSPK_TEXT_ENGINE[299]);
			}
			CustomFont.Draw(CustomFont.FontNormal, x + 14 + 75 * 4, y + h - 62.5, 0xffffffff, 0x0, 73, 0, 3, skin->Skin != this->Skin ? gConfig.TSPK_TEXT_ENGINE[300] : gConfig.TSPK_TEXT_ENGINE[301]);
		}
		else
		{
			CustomFont.Draw(CustomFont.FontNormal, x + 80 * 2, y + 35, 0x0, 0xFF8400FF, 150, 10, 0, " ");
			CustomFont.Draw(CustomFont.FontNormal, x + 70 * 2, y + 35, 0xffffffff, 0x0, 150, 0, 3, gConfig.TSPK_TEXT_ENGINE[302]);
			CustomFont.Draw(CustomFont.FontNormal, x + 14 + 75 * 4, y + h - 62.5, 0xffffffff, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[303]);
		}
	}
	else
	{
